...
# MotorcycleTrajectory state machine constants (new model)
_TURN_ANGLES_RAD = None  # No longer a class attribute - turn angles are random now
...
class TestPedestrianTrajectory:
    ...
    def test_altitude_zero(self):
        ...
        for _e, _n, u in _run_trajectory(ped, 100):
            assert u == 0.0, "Pedestrian altitude should always be 0"

    def test_heading_drift_is_smooth(self):
        """Heading changes must be smooth per step (OU process model, ~10°/step typical)."""
        ...
        # OU process: typical ~5°/step, occasional 10°+ allowed for realistic turns
        assert max_diff_deg <= 15.0, f"Heading changed too sharply: {max_diff_deg:.2f}°"
...
class TestMotorcycleTrajectory:
    ...
    def test_altitude_zero(self):
        ...
        for _e, _n, u in _run_trajectory(moto, 100):
            assert u == 0.0
    ...
    def test_turn_arc_geometry(self):
        ...
        # All heading diffs should have the same sign (monotonic turn)
        positive = sum(1 for d in diffs if d > 0.01)
        negative = sum(1 for d in diffs if d < -0.01)
        total = len(diffs)
        # At least 90% should be same direction (some noise from steering transitions)
        assert max(positive, negative) >= total * 0.9, (
            f"Turn heading not monotonic: {positive} positive, {negative} negative out of {total}"
        )
...
class TestSimulationBoundary:
    ...
    def test_reflection_heading_points_inward(self):
        """After soft repulsion, heading should be biased toward centre."""
        b = SimulationBoundary(radius_m=100.0)
        ...
        # constrain clamps to (100, 0) and biases heading toward centre
        ...
        # With soft repulsion, heading should be biased toward centre
        # Original heading was East (pi/2), centre is West (atan2(-100, 0) = -pi/2)
        # The new heading should have shifted toward West
        to_center = math.atan2(-e, -n)  # heading toward centre
        diff = (to_center - new_h + math.pi) % (2 * math.pi) - math.pi
        # Heading should be closer to centre than original
        assert abs(diff) < abs(to_center - heading_east), "Heading should be biased toward centre"
    ...
    def test_on_boundary_exact(self):
        ...
        b = SimulationBoundary(radius_m=100.0)
        e, n, _h = b.constrain(100.0, 0.0, math.pi)  # heading North (inward)
...
class TestAdaptiveR2D:
    ...
    def test_step_with_sigma_pos_updates_R(self):
        ...
        kf.step(0.0, 0.0)
        # Second call estimates velocity from finite difference (no R update)
        kf.step(1.0, 0.0, sigma_pos_m=3.0)
        # Third call runs full predict+update WITH sigma_pos_m
        kf.step(2.0, 0.0, sigma_pos_m=3.0)
...
class TestMotorcycleNoCrash:
    ...
    and that the new bicycle kinematic model is stable.
    """

...
Giao diện được tổ chức thành các vùng chức năng:

\begin{itemize}
    \item \textbf{Vùng điều khiển mô phỏng (Simulation Panel):} Chọn loại mục tiêu (PEDESTRIAN/MOTORCYCLE/DRONE), thuật toán lọc (KF+AB/KF ONLY/AB ONLY), thời lượng (giây), bán kính ranh giới (mét), và nguồn dữ liệu (Synthetic/Real-World Data). Nút ENGAGE bắt đầu mô phỏng.

    \item \textbf{Bản đồ (Tracking Map):} Hiển thị bản đồ OpenStreetMap với các lớp dữ liệu (quỹ đạo thô, Kalman, alpha-beta, đường viền). Bán kính ranh giới 200 m, 400 m, 600 m hiển thị dưới dạng đường tròn bán trong suốt.

    \item \textbf{Biểu đồ sai số (Error Metrics Chart):} Biểu đồ đường thể hiện sai số vị trí theo thời gian cho cả hai bộ lọc và đo thô.

    \item \textbf{Biểu đồ cao độ (Altitude Chart):} (Chỉ cho drone) Hiển thị cao độ thực tế, ước lượng Kalman và alpha-beta theo thời gian.

    \item \textbf{Bảng số liệu (Status Bar):} Hiển thị tọa độ hiện tại (East, North, Alt), sai số RMSE theo từng bộ lọc, số frame, và trạng thái hệ thống.
\end{itemize}
...
\hspace{0.5cm} Mỗi phiên tracking được quản lý độc lập trên server, với vòng đời gồm bốn giai đoạn: khởi tạo, chạy, tạm dừng, và dọn dẹp. Bảng \ref{tab:session-lifecycle} mô tả chi tiết từng giai đoạn và các thao tác associated.
...
\toprule
\textbf{Giai đoạn} & \textbf{Kích hoạt} & \textbf{Thao tác} \\
\midrule
Khởi tạo & ENGAGE button & Tạo session ID, khởi tạo 3 bộ lọc (KF, AB, raw), cấp phát ring buffer 500 điểm, bắt đầu mô phỏng \\
Chạy & Tự động sau khởi tạo & Mô phỏng tạo quỹ đạo, bộ lọc xử lý, gửi bản tin 10 Hz, ghi ring buffer \\
Tạm dừng & Disconnect / lỗi mạng & Dừng mô phỏng, giữ trạng thái bộ lọc, ring buffer giữ dữ liệu lịch sử \\
Dọn dẹp & Đóng tab / STOP & Xóa session, giải phóng bộ lọc, dừng mô phỏng, xóa ring buffer \\
\bottomrule
\end{tabular}
}
\caption{Vòng đời phiên tracking}
\label{tab:session-lifecycle}
\end{table}

...
Bảng \ref{tab:elevation-edge} xác minh hành vi ở các góc elevation đặc trưng với $R = 100$ m và $\psi_{az} = 0°$ (hướng Bắc).

\begin{table}[H]
\centering
\caption{Kết quả chuyển đổi polar sang ENU với $R = 100$ m, $\psi_{az} = 0°$, các góc elevation khác nhau}
...
\midrule
$+30°$ & 0{,}00  & 86{,}60  & $+$50{,}00 & Mục tiêu cao hơn người quan sát \\
$0°$   & 0{,}00  & 100{,}00 &    0{,}00  & Cùng mặt phẳng ngang \\
$-15°$ & 0{,}00  & 96{,}59  & $-$25{,}88 & Mục tiêu thấp hơn người quan sát \\
$-45°$ & 0{,}00  & 70{,}71  & $-$70{,}71 & Góc ngắm xuống dốc \\
$-90°$ & 0{,}00  &  0{,}00  & $-$100{,}00 & Mục tiêu thẳng đứng xuống \\
\bottomrule
\end{tabular}
}
\end{table}
...
Trong hầu hết trường hợp mô phỏng, $\sigma_U$ nhỏ hơn $\sigma_E$ và $\sigma_N$ đáng kể vì khoảng cách laser $R$ ảnh hưởng trực tiếp đến độ chính xác elevation nhưng thành phần GPS (độc lập với elevation) vẫn chiếm ưu thế. Tuy nhiên, khi elevation âm lớn (góc ngắm xuống dốc hơn $-30°$), thành phần $R \sin(\theta_{el})$ tăng nhanh và có thể trở thành nguồn sai số đáng kể nếu khoảng cách laser không được hiệu chuẩn đúng.
...
Độ nhiễu cảm biến trong chế độ Synthetic được mô hình hóa bằng phân phối Gaussian với các tham số cố định: $\sigma_{GPS} = 5{,}0$ m, $\sigma_{az} = 0{,}3°$, $\sigma_{el} = 0{,}2°$, $\sigma_{range} = 0{,}5$ m.
...
\begin{table}[H]
\centering
\caption{Xử lý trường hợp đặc biệt trong xuất CSV}
...
\midrule
Cao độ drone âm & Mô hình drone tạo altitude nhỏ hơn 0 & Bảng chuyển đổi polar-to-ENU dừng elevation at $-90°$, altitude được clamp về 0 \\
Ring buffer chưa đủ 500 khung & Phiên chạy ngắn (< 50 giây) & Xuất toàn bộ nội dung buffer, không cần ring wrap \\
Mục tiêu chạm biên & Mô hình phản lực đẩy mềm & Vị trí thực vẫn được ghi, không có gap trong CSV \\
WebSocket ngắt giữa chừng & Lỗi mạng hoặc đóng tab & File CSV vẫn đầy đủ đến khung cuối cùng \\
Seed không xác định & Phiên dùng seed=None & Cột timestamp khác giữa các lần chạy, cần so sánh thủ công \\
\bottomrule
\end{tabular}
}
\end{table}

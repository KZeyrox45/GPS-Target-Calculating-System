...
% 2. Pause & Resume (Đẩy nhãn lệch về hai phía để tạo khoảng trống ở giữa)
\draw[arrow] ([yshift=3.5mm]running.west) -- node[lbl, pos=0.25, above=1pt] {PAUSE\_CMD} ([yshift=3.5mm]paused.east);
\draw[arrow] ([yshift=-3.5mm]paused.east) -- node[lbl, pos=0.4, below=1pt] {RESUME\_CMD} ([yshift=-3.5mm]running.west);
...
% 5. Init failure (Uốn nhẹ để tách biệt hoàn toàn khỏi đường chéo còn lại)
\draw[arrow] (init.south west) to[bend right=15] node[lbl, pos=0.3, sloped, above=1pt] {INIT\_FAILED} ([xshift=6mm]error.north);
...
\resizebox{\linewidth}{!}{
\begin{tikzpicture}[
    participant/.style={rectangle, draw=black!80, fill=blue!10, minimum width=2.4cm, minimum height=0.8cm, align=center, font=\small\sffamily, thick},
    arrow/.style={->, >=stealth, thick},
    reply/.style={<-, >=stealth, dashed, thick},
    lifeline/.style={dashed, gray!80, line width=0.8pt},
    msg/.style={font=\scriptsize\sffamily, above}
]

% Participants
\node[participant] (ui) at (0, 7) {Web Frontend};
\node[participant] (api) at (3.8, 7) {REST API Router};
\node[participant] (engine) at (7.6, 7) {Simulation Core};
\node[participant] (ws) at (11.4, 7) {WebSocket Router};

% Lifelines
\draw[lifeline] (ui) -- (0, 0);
\draw[lifeline] (api) -- (3.8, 0);
\draw[lifeline] (engine) -- (7.6, 0);
\draw[lifeline] (ws) -- (11.4, 0);

% Messages
\draw[arrow] (0, 6.0) -- node[msg] {POST /api/simulation/start} (3.8, 6.0);
\draw[arrow] (3.8, 5.7) -- node[msg] {Instantiate Session Engine} (7.6, 5.7);
\draw[arrow] (7.6, 5.2) -- node[msg] {Alloc Session Buffer} (7.6, 4.8);
\draw[reply] (3.8, 4.4) -- node[msg] {Session ID + WS URL} (7.6, 4.4);
\draw[reply] (0, 4.0) -- node[msg] {HTTP 200 OK (ws\_url)} (3.8, 4.0);

\draw[arrow] (0, 3.4) -- node[msg] {WS Upgrade Handshake} (11.4, 3.4);
\draw[reply] (0, 3.0) -- node[msg] {101 Switching Protocols} (11.4, 3.0);

\draw[arrow] (11.4, 2.5) -- node[msg] {Start 10 Hz Frame Loop} (7.6, 2.5);
\draw[arrow] (7.6, 2.0) -- node[msg] {Push TrackingFrame (100 ms)} (11.4, 2.0);
\draw[arrow] (11.4, 1.5) -- node[msg] {Stream Frame JSON} (0, 1.5);
\draw[arrow] (0, 0.9) -- node[msg] {Render Leaflet Map \& Charts} (0, 0.5);
...
\begin{table}[H]
\centering
\caption{Phân tầng cấu trúc 162 ca kiểm thử tự động toàn diện của hệ thống}
...
\midrule
Tầng 1: Đơn vị (Unit Tests) & Giải thuật Kalman, Alpha-Beta, Geodetics, RSS & 98 & 60{,}5\% \\
\addlinespace[3pt]
Tầng 2: Tích hợp (Integration) & Bộ tải dữ liệu, Chuỗi nối tầng LLA-ENU, Ranh giới & 42 & 25{,}9\% \\
\addlinespace[3pt]
Tầng 3: Đầu-cuối (End-to-End) & Luồng REST-WS, Xuất tệp CSV, Dashboard Telemetry & 22 & 13{,}6\% \\
\midrule
\textbf{Tổng cộng toàn hệ thống} & \textbf{Toàn diện 3 tầng kiến trúc} & \textbf{162} & \textbf{100{,}0\% (Đạt 100\%)} \\
\bottomrule
\end{tabularx}
\end{table}
...
\hspace{0.5cm} Nhằm bảo đảm tính khả chuyển cao nhất khi chuyển giao hệ thống từ môi trường mô phỏng sang các thiết bị máy tính nhúng tại hiện trường (như Raspberry Pi 4/5 hay NVIDIA Jetson Nano), kiến trúc phần mềm được kiểm tra tính tương thích trên ba hệ điều hành phổ biến:
...
\begin{tabular}{lccc}
\toprule
\textbf{Tiêu chí kỹ thuật} & \textbf{Windows 11 Pro (x86\_64)} & \textbf{Ubuntu 22.04 LTS (x86\_64)} & \textbf{Raspberry Pi OS (ARM64)} \\
\midrule
Thời gian khởi động hệ thống & $2{,}1$ s & $1{,}8$ s & $3{,}4$ s \\
Chu kỳ tính toán một khung đo & $59{,}0$ $\mu$s & $48{,}2$ $\mu$s & $142{,}5$ $\mu$s ($< 100$ ms) \\
Tỷ lệ kiểm thử tự động đạt & $162 / 162$ ($100\%$) & $162 / 162$ ($100\%$) & $162 / 162$ ($100\%$) \\
Chiếm dụng RAM tĩnh khi nạp đồ thị & $245$ MB & $238$ MB & $252$ MB \\
Độ ổn định kênh WebSocket 10 Hz & Tuyệt đối (0 lỗi) & Tuyệt đối (0 lỗi) & Tuyệt đối (0 lỗi) \\
\bottomrule
\end{tabular}
}
\end{table}

Trên kiến trúc phần cứng nhúng ARM64 (Raspberry Pi 4), tổng thời gian tính toán một khung chỉ mất $142{,}5$ $\mu$s (chưa đến $0{,}15$ ms), vẫn dư thừa hơn $99{,}8\%$ chu kỳ định kỳ 100 ms của tần số 10 Hz, khẳng định hệ thống hoàn toàn sẵn sàng cho việc nhúng trực tiếp lên các thiết bị phần cứng thực địa trong tương lai.

\subsection{Tổng kết các kết quả đạt được}

\hspace{0.5cm} Công tác tích hợp hệ thống đầu-cuối trong tuần 11 đã đạt được những mốc chất lượng quan trọng:
\begin{enumerate}
    ...
    \item Đạt $100\%$ tỷ lệ thành công trên toàn bộ 162 ca kiểm thử tự động, xác nhận độ tin cậy tuyệt đối của hệ thống bám mục tiêu.
\end{enumerate}

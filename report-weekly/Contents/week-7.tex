...
Việc áp dụng cùng một hạt giống ngẫu nhiên cho cả ba phương pháp xử lý trong từng kịch bản bảo đảm tính tương thích cặp (paired comparison), loại bỏ hoàn toàn sai số thiên vị do quỹ đạo khác nhau khi so sánh đối đầu giữa bộ lọc Kalman và bộ lọc Alpha-Beta.

\begin{table}[H]
\centering
\caption{Bộ tham số cấu hình chuẩn cho đánh giá thống kê đa giá trị khởi tạo}
\label{tab:sim-statistical-config}
...
Bảng \ref{tab:ecdf-quantiles} thống kê xác suất tích lũy sai số $P(\varepsilon \le r)$ tại các mốc bán kính ngưỡng $r$ từ $0{,}5$ m đến $5{,}0$ m thu được từ toàn bộ $108.000$ điểm dữ liệu thực nghiệm qua 10 seed.

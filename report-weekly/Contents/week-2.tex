...
\draw[arrow] (firmware) -- node[midway, left=5pt, font=\footnotesize] {UART/$I^2C$} (gps);
\draw[arrow] (firmware) -- node[midway, right=2pt, font=\footnotesize] {$I^2C$} (imu);
...
\begin{itemize}
    ...
    \item Gửi dữ liệu lên máy chủ qua WebSocket (Wi-Fi hoặc Ethernet).
    \item (Tương lai) Thực hiện xử lý sơ bộ như lọc nhiễu, kiểm tra lỗi.
\end{itemize}

\subsubsection{Tầng máy chủ (Server layer)}

Máy chủ được xây dựng bằng FastAPI, một framework Python hiện đại hỗ trợ bất đồng bộ (async). Các chức năng:
\begin{itemize}
    \item \textbf{REST API}: Cho phép khởi tạo phiên (session), dừng phiên, truy vấn lịch sử dữ liệu. Định dạng trả về là JSON.
    \item \textbf{WebSocket}: Kênh giao tiếp hai chiều thời gian thực. Raspberry Pi gửi dữ liệu cảm biến lên, server xử lý và gửi kết quả xuống frontend.
...
Giao diện người dùng chạy trên trình duyệt web, sử dụng React và Vite để đóng gói. Các thành phần chính:
\begin{itemize}
    \item \textbf{Bản đồ}: Thư viện Leaflet hiển thị bản đồ OpenStreetMap. Hiển thị vị trí người quan sát (marker xanh) và vị trí mục tiêu (marker đỏ). Leaflet hiện tại đáp ứng tốt cho bản đồ 2D. Nhóm đang nghiên cứu tính khả thi của việc nâng cấp lên bản đồ 3D sử dụng CesiumJS.
    ...
    \item \textbf{Điều khiển phiên}: Các nút bật/tắt tracking, cấu hình tham số.
\end{itemize}

\subsection{Nguyên lý hoạt động}

Hoạt động của hệ thống được mô tả bằng các bước sau:
...
\end{verbatim}
    \item Bản tin JSON được gửi lên server qua WebSocket.
    \item Server nhận dữ liệu, thực hiện:
    \begin{itemize}
    ...
    \end{itemize}
    \item Tọa độ mục tiêu đã xử lý được gửi xuống frontend qua WebSocket.
    \item Frontend cập nhật vị trí mục tiêu trên bản đồ.
\end{enumerate}
...
Hệ thống có hai quy trình hoạt động chính được mô tả bằng các activity diagrams.
...
Raspberry Pi được chọn làm nền tảng xử lý nhúng vì những lý do sau:
\begin{itemize}
...
\hspace{-0.5cm} Thiết kế dự kiến:
\begin{itemize}
    \item GPS module kết nối qua chân UART (pin GPIO 14/15). Đọc dữ liệu NMEA bằng thư viện \texttt{pyserial}.
    \item IMU kết nối qua I2C (pin GPIO 2/3). Đọc dữ liệu bằng thư viện \texttt{smbus2} hoặc \texttt{BNO085}.
    \item Laser rangefinder kết nối qua UART hoặc I2C.
    \item Ứng dụng Python chạy độc lập, đọc cảm biến theo chu kỳ, gửi dữ liệu qua WebSocket bằng thư viện \texttt{websockets}.
\end{itemize}

Phần xử lý nhúng này được thiết kế để có thể hoạt động độc lập, không cần máy tính. Người dùng mang Raspberry Pi (gắn trên bảng điều khiển, tripod, hoặc túi đeo) ra hiện trường, bật nguồn là hệ thống tự động chạy. Dữ liệu được gửi về máy chủ trung tâm qua mạng Wi-Fi (hoặc 4G nếu có USB dongle).

Tuy nhiên, hiện tại nhóm chưa có đủ thiết bị phần cứng để triển khai phần này. Các thử nghiệm bước đầu được thực hiện trên máy tính cá nhân, sử dụng dữ liệu mô phỏng (simulation) thay cho cảm biến thật. Vị trí GPS được tạo ngẫu nhiên trong một bán kính cho trước, góc ngắm và khoảng cách được mô phỏng dựa trên quỹ đạo của mục tiêu giả định. Phần xử lý trên Raspberry Pi sẽ được triển khai trong giai đoạn sau khi có đủ thiết bị.

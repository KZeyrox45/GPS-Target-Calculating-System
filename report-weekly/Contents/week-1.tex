...
Giai đoạn 2 mở rộng bài toán theo một hướng khác. Mục tiêu không còn đứng yên mà có thể di chuyển. Hệ thống phải đảm bảo tính toán liên tục, cập nhật vị trí mục tiêu theo thời gian thực, và hiển thị quá trình di chuyển trên bản đồ số. Điều này đòi hỏi phải xử lý dòng dữ liệu từ nhiều cảm biến cùng lúc, kết hợp chúng với nhau, và áp dụng các bộ lọc để ước lượng vị trí chính xác ngay cả khi tín hiệu bị nhiễu hoặc gián đoạn.

Để đạt được điều đó, nhóm mở rộng kiến trúc hệ thống theo hướng sau:
\begin{itemize}[label={}]
    ...
    \item (iii) Xây dựng hệ thống truyền thông thời gian thực bằng WebSocket.
    \item (iv) Xây dựng giao diện bản đồ tương tác trên nền web.
\end{itemize}
\hspace{0.5cm} Ngoài ra, hệ thống sẽ được triển khai trên Raspberry Pi để xử lý tại thiết bị di động trước khi gửi lên máy chủ trung tâm. Tuy nhiên, phần cứng hiện chưa có sẵn nên giai đoạn này sẽ thực hiện sau.
...
\hspace{-0.5cm} Các ràng buộc chính của bài toán:
...
Hệ thống được đề xuất gồm bốn tầng chính:
...
\begin{itemize}
    ...
    \item Phát dữ liệu đã xử lý xuống frontend qua WebSocket.
\end{itemize}

\subsubsection{Tầng giao diện (Frontend Layer)}

\hspace{0.5cm} Giao diện người dùng được xây dựng bằng React kết hợp với Vite, sử dụng thư viện Leaflet để hiển thị bản đồ. Các chức năng chính bao gồm:
\begin{itemize}
    ...
    \item Hiển thị các thông số đo lường (tọa độ, góc, khoảng cách).
    \item Điều khiển phiên (bắt đầu, dừng, cấu hình).
\end{itemize}

\subsection{Cơ sở lý thuyết mở rộng}

\hspace{0.5cm} Trong giai đoạn 1, nhóm đã trình bày các kiến thức nền tảng về GPS, tam giác cầu, các phép chiếu bản đồ, và các thuật toán geodesic (Haversine, Vincenty, Karney). Ở giai đoạn 2, cần mở rộng thêm các khái niệm sau:
...
\hspace{0.5cm} Điểm mới của đề tài này là phải kết hợp dữ liệu từ nhiều cảm biến khác nhau. Lý do là:
\begin{itemize}
    \item GPS cho vị trí tuyệt đối nhưng tần số thấp (1-10 Hz) và dễ bị mất tín hiệu.
    \item IMU cho tần số cao (100-1000 Hz) nhưng bị trôi (drift) theo thời gian.
    \item Laser cho khoảng cách chính xác nhưng cần có LOS (Line of Sight).
\end{itemize}

Kết hợp ba nguồn này cho phép hệ thống ước lượng vị trí mục tiêu ngay cả khi một hoặc hai cảm biến bị suy giảm. Ví dụ, nếu GPS mất tín hiệu, hệ thống vẫn có thể ước lượng vị trí từ IMU và laser cho đến khi GPS hoạt động trở lại.

\subsubsection{Kalman filter}

\hspace{0.5cm} Kalman filter là thuật toán cổ điển trong điều khiển và dẫn đường, được Rudolf Kalman công bố năm 1960. Nó cho phép ước lượng trạng thái của một hệ thống từ các đo lường nhiễu và mô hình động học. Trong bài toán tracking, Kalman filter được sử dụng để ước lượng vị trí và tốc độ của mục tiêu.\\
...
Với mục tiêu drone cần theo dõi cả trục Up, vector trạng thái mở rộng thành 6 chiều $\begin{bmatrix} e & n & u & \dot{e} & \dot{n} & \dot{u} \end{bmatrix}^T$ và ma trận $\mathbf{F}$ mở rộng tương ứng thành $6\times 6$ (\texttt{KalmanFilter3D}).

\subsubsection{Alpha-beta filter}

\hspace{0.5cm} Alpha-beta filter (còn gọi là g-h filter) là phiên bản đơn giản hóa của Kalman filter, chỉ dùng hai tham số: alpha (vị trí) và beta (vận tốc). Nó phù hợp cho các hệ thống có tài nguyên tính toán hạn chế như Raspberry Pi, và vẫn cho kết quả tốt với các mục tiêu di chuyển đều.
...
\hspace{0.5cm} Hình \ref{fig:usecase} mô tả sơ đồ use-case của hệ thống. Người dùng (người quan sát) có thể khởi tạo phiên tracking, cấu hình tham số, xem lộ trình, hoặc dừng phiên. Hệ thống tự động cập nhật dữ liệu cảm biến, tính toán tọa độ, và hiển thị trên bản đồ. Khi mất tín hiệu, hệ thống sẽ đưa ra cảnh báo.
